...
function CartSidebar() {
  const { auth } = useAuth();
  const { cart, openCartSidebar, changeOpenCartSiderbar } = useUserData();

  return (
    <>
      <div
        onClick={changeOpenCartSiderbar}
        className={`${styles.fondoCartSidebar} ${
          openCartSidebar && styles.openFondoCartSidebar
        }`}
      ></div>

      <div
        className={`${styles.cartSidebar} ${
          openCartSidebar && styles.openCartSidebar
        }`}
      >
        ...
        {Object.values(auth).length === 0 ? (
          <>
            <p className={styles.textoCarrito}>
              Ingresa para empezar a agregar productos
            </p>
