...
function Producto({ productoProp }) {
  ...
  const { titulo, imagen, precio, descripcion, color_franela, id } =
    productoProp[0];
  ...
  const handleAgregarCarrito = () => {
    ...
    const productId = `${id}-${damaCaballero ? "Dama" : "Caballero"}-${
      color_franela.Color
    }-${tallaSelected}`;
  ...
  const { msg } = alerta;
  return (
    <Layout pagina={titulo} headerProps={false}>
          ...
          <div className={styles.detallesProducto}>
                ...
                {damaCaballero ? (
                  tallasDama.length === 0 ? (
                    <p className={styles.noDisponible}>
                      {" "}
                      No hay tallas de Dama Disponibles
                    </p>
                  ) : (
                    tallasDama.map((elemento) => (
                      <button
                        ...
                        className={
                          tallaSelected === elemento.talla
                            ? styles.tallaSeleccionada
                            : null
                        }
                      >
                        {elemento.talla}
                  ...
                  <p className={styles.noDisponible}>
                    {" "}
                    No hay tallas de Caballero Disponibles
                  </p>
                ) : (
            ...
            <div className={styles.cantidadAgregarCarrito}>
              ...
              {Object.values(auth).length !== 0 && tallaSelected !== "" ? (
                ...
                <button
                  disabled={true}
                  className={`${styles.agregarAlCarrito} ${styles.inactivo}`}
                >
                  Agregar al carrito
                </button>
              )}
            ...
            {Object.values(auth).length === 0 && (
              <Link href="/mi-cuenta/acceder-registrarse">
                <a className={styles.accederParaCarrito}>
                  Accede Para Comenzar a Agregar Productos
                </a>
              </Link>
            )}
